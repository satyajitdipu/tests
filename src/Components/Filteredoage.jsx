...
const Filteredoage = () => {
  const { occupation } = useParams(); // Extract 'occupation' from the URL
  ...
  useEffect(() => {
    ...
    if (occupation === '0') {
        handleCategoryClick("SLP");
      } else if (occupation === '3') {
        handleCategoryClick("MIND");
      } else if (occupation === '4') {
        handleCategoryClick("OT");
      }
    AOS.init({ duration: "1500" });
    // Fetch initial data whenever category or age filter changes
    fetchGamesData();
  }, [selectedAgeFilter, selectedCategory, occupation]); // Updated dependencies
