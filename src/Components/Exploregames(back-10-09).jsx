...
const Exploregames = () => {
  ...
  const [active_btn, setactive_btn] = useState('')
  const navigate = useNavigate();

  // Fetch games, categories, and age filters from API
  const fetchGamesData = async () => {
    try {
      console.log("Fetching data with:", { selectedAgeFilter, selectedCategory }); // Log parameters
      const response = await fetch(`https://virtualtxai.com/api/all-games?age_filters=${selectedAgeFilter}&categories=${selectedCategory}`);
  ...
  useEffect(() => {
    // Fetch initial data
    fetchGamesData();
  }, [selectedAgeFilter, selectedCategory]); // Added dependencies

  const handleCategoryClick = (category) => {
    setSelectedCategory(category);
    setactive_btn(category)
  };
  ...
  return (
    <div>
          ...
          <div className="col-xl-2 col-lg-2 col-md-2 col-12">
            ...
            <div className="col-xl-2 col-lg-2 col-md-2 col-12">
              <button
                ...
                className="btn py-xl-3 py-lg-3 py-md-2 py-2 my-xl-0 my-lg-0 my-md-0 my-2 grey_bg text-center rounded w-100 mb-2"
              >
                OT
              </button>
            </div>
      ...
      {/* Category 2 */}
      <section className={toggle ? "bg-grey p-xl-3 p-lg-3 p-md-3 p-2" : "bg-grey p-xl-3 p-lg-3 p-md-3 p-2 d-none"}>
        <div className="container">
          <div className="row">
            {categories.map((category) => (
              <div key={category.id} className="col-xl-2 col-lg-2 col-md-3 col-12 mt-xl-2 mt-lg-2 mt-md-2 mt-1">
                <div
        ...
        <div className="container">
          <div className="row d-flex justify-content-between align-items-center">
            <div className="col-xl-3 col-lg-3 col-md-3 col-12">
              <h3 className="fw-bold py-xl-3 py-lg-3 py-md-2 py-2">All Games</h3>
            </div>
          </div>
        </div>
      </section>
